...
const Home = () => {
    const totalConsultAvg = Math.round(
        users.reduce((sum, user) => sum + user.totalConsults, 0) / users.length
    );
    const frequencyAvg = Math.round(
        users.reduce((sum, user) => sum + user.frequency, 0) / users.length
    );
    const totalInteractionAvg = Math.round(
        users.reduce((sum, user) => sum + user.totalInteractions, 0) / users.length
    );
    return (
